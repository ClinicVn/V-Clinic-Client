...
public class UserService {
	...
	public List<ClinicMessage> save(int mode, JsonNode jUser, String token) {
		...
		if (mode == 1)
			jReturn = ServiceUrl.post(req, jUser);
		else
			jReturn = ServiceUrl.put(req, jUser);
		if (jReturn == null) {
			lstError.add(new ClinicMessage("Exception", StringValue.ERR00000));
		} else if (jReturn.has("error")) {
			lstError.add(new ClinicMessage("userCode", jReturn.get("error")
					.asText()));
		}
		return lstError;
	}

	public List<ClinicMessage> validCreateInfo(int mode, JsonNode dataNode,
			String token) {
		List<ClinicMessage> lstError = new ArrayList<ClinicMessage>();
		if (mode == 1) {
			// user validate
			...
			if (password.length() < 5 || password.length() > 50) {
				lstError.add(new ClinicMessage("userPwd",
						"Password's length must be between 5 and 50 !"));
			} else if (!password.equals(rePassword)) {
				lstError.add(new ClinicMessage("rePassword",
		...
		if (name.trim().length() == 0) {
			lstError.add(new ClinicMessage("fullname",
					"Fullname must be input !"));
		}
		// valid email
